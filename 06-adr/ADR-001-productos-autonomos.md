...
VGM Sistemas desarrolla tres productos en paralelo: VGM Core (ERP), VGM Go (geolocalización) y VGM GEMA (preventa móvil). La pregunta era si estos productos debían compartir módulos, base de datos o autenticación.
...
|---|---|
| VGM Core como plataforma central con auth compartida | Crea dependencia: VGM Go no puede funcionar ni venderse sin VGM Core |
...
**Positivas:**
- VGM Go se puede vender a clientes que no usan VGM Core
- Cada producto puede evolucionar a su propio ritmo
- Un bug en VGM Core no afecta a VGM Go en producción
...
**Mitigaciones:**
- Campos `id_publico_core` en tablas clave de VGM Go para facilitar la integración futura
